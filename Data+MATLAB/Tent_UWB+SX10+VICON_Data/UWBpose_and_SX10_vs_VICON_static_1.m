...
%% Analyze rosbag data
Bag = rosbag('2021-02-25-20-27-57.bag');
...
error_angle_uwb = angle_vicon2uwb_deg(:,3) - angle_uwb2vicon_deg(:,3);
error_mean_angle_uwb = mean(error_angle_uwb);
error_std_angle_uwb = std(error_angle_uwb);

mean_x_vicon = mean(x_vicon);
mean_y_vicon = mean(y_vicon);
std_x_vicon = std(x_vicon);
std_y_vicon = std(y_vicon);
error_euclidean_vicon = sqrt((x_vicon - mean_x_vicon).^2 + (y_vicon - mean_y_vicon).^2);
error_mean_vicon = mean(error_euclidean_vicon);
error_std_vicon = std(error_euclidean_vicon);

mean_x_uwb = mean(x_UWBpose);
mean_y_uwb = mean(y_UWBpose);
std_x_uwb = std(x_UWBpose);
std_y_uwb = std(y_UWBpose);

mean_x_trimble = mean(x_trimble_raw);
mean_y_trimble = mean(y_trimble_raw);
std_x_trimble = std(x_trimble_raw);
std_y_trimble = std(y_trimble_raw);

static_transform_uwb2vicon = [abs(mean_x_vicon - mean_x_uwb); abs(mean_y_vicon - mean_y_uwb)];
static_transform_trimble2vicon = [abs(mean_x_vicon - mean_x_trimble); abs(mean_y_vicon - mean_y_trimble)];
...
figure
plot(vicon(1,:),vicon(2,:),'.')
hold on
plot(uwb2vicon_transformed(1,:),uwb2vicon_transformed(2,:),'.')
grid on
...
plot(vicon(1,:),vicon(2,:),'.')
hold on
plot(trimble2vicon_transformed(1,:),trimble2vicon_transformed(2,:),'.')
grid on
...
figure
quiver(x_vicon,y_vicon,u_vicon, v_vicon, 0);
hold on
quiver(x_UWBpose,y_UWBpose,u_UWBpose, v_UWBpose, 0);
axis equal
